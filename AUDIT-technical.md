...
**Evidence (live):**
```
curl -s 'https://linespedia.com/' | grep -oi '<link rel="alternate"[^>]*hreflang[^>]*>'
```
...
- Canonical tag: `<link rel="canonical" href="https://linespedia.com/">` (correct)
...
**Evidence (live):**
```
curl -sI https://linespedia.com | grep -i strict-transport
```
Returns: (empty)
...
```
Sitemap: https://linespedia.com/sitemap.xml
```
...
**Fix:** Reconcile the local and live robots.txt. The live Cloudflare-managed version should include the more comprehensive rules from the local file, and the sitemap reference should consistently point to `https://linespedia.com/sitemap.xml`.
...
**Evidence (live):**
```
curl -s 'https://linespedia.com/?utm_source=test' | grep -oi 'noindex'
```
...
**Fix:** In `/Users/shaswatraj/Desktop/earn/linepedia/src/pages/index.astro`, add a check for tracking/search parameters and merge the noindex result into the `isTranslationParamPage` logic. The canonical tag is correctly set to `https://linespedia.com/` (without params), which is good, but `noindex` should accompany it.
